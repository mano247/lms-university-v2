...
public class UniverzitetController {
	...
	public ResponseEntity<Iterable<UniversityDTO>> getAll(){
		HashSet<UniversityDTO> university = new HashSet<UniversityDTO>();
		for (University u : service.findAll()) {
			...
			university.add(new UniversityDTO(u.getId(), u.getName(), u.getFoundingDate(), u.getDescription() , u.getContact(),  u.getImage(), u.getAddress(), rectorate));
		}
		return new ResponseEntity<Iterable<UniversityDTO>>(university, HttpStatus.OK);
	}
